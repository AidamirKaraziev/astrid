"""Колода таро: каркас на 78 карт, контент v1 — 22 старших аркана.

Структура сразу под будущую игру: масти, поле keywords_reversed
(в v1 пустое — перевёрнутые не используются), astro_affinity — традиционные
планетарно-знаковые соответствия арканов (связка с транзитами в интерпретации).
"""
...
class TarotCard:
    ...
    astro_affinity: str  # планета/знак по традиционной атрибуции, по-русски
    voice: str  # 1–2 фразы «голоса карты» для промпта
    keywords_reversed: tuple[str, ...] = field(default=())  # v1 не используется
    image_file: str | None = None  # этап 2: ассеты Rider-Waite
...
# v1: тянем только из карт с контентом; каркас позволит добавить 56 младших
DECK: tuple[TarotCard, ...] = MAJOR_ARCANA
