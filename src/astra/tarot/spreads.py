...
class SpreadSpec:
    ...
    question_hint: str  # подсказка при вводе вопроса; отправляется с parse_mode=HTML
    max_tokens: int
...
SPREADS: dict[SpreadType, SpreadSpec] = {
    SpreadType.WISH: SpreadSpec(
        ...
        # Подсказка читается «одним взглядом»: сверху действие, ниже примеры.
        # Тему желания задают сами примеры — перечислять её словами не нужно.
        question_hint=(
            "✍️ <b>Напиши своё желание</b> — одним сообщением\n\n"
            "Например:\n"
            "<i>«Хочу, чтобы он снова написал мне первым»</i>\n"
            "<i>«Хочу выйти на новый доход к осени»</i>"
        ),
        max_tokens=700,
    ),
    SpreadType.THREE_CARDS: SpreadSpec(
        ...
        question_hint=(
            "✍️ <b>Напиши свой вопрос</b> — одним сообщением\n\n"
            "Например:\n"
            "<i>«Стоит ли менять работу этим летом?»</i>\n\n"
            "Не знаешь, что спросить — жми «⏭ Пропустить»"
        ),
        max_tokens=750,
    ),
    SpreadType.RELATIONSHIP: SpreadSpec(
        ...
        question_required=True,
        question_hint=(
            "✍️ <b>Напиши, кто он(она) тебе и что между вами</b>\n\n"
            "Например:\n"
            "<i>«Мой бывший, месяц не общаемся после ссоры»</i>\n\n"
            "Карты покажут, что он(она) чувствует — даже то, о чём молчит."
        ),
        max_tokens=1150,
    ),
}
